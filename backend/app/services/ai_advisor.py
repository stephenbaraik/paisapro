...
async def _execute_tool(name: str, arguments: dict) -> str:
    ...
    try:
        if name == "get_stock_analysis":
            ...
            from .market_data import get_nifty_df
            import numpy as np
...
async def stream_advisor_response(request: AdvisorChatRequest):
    ...
    for _round in range(MAX_TOOL_ROUNDS):
        ...
        logger.info(
            "Phase1 round=%d finish=%s tool_calls=%s content_len=%d",
            _round,
    ...
    for i, word in enumerate(words):
        token = word if i == 0 else " " + word
        yield f"data: {_json.dumps({'token': token})}\n\n"
        # Small delay every few words to create streaming feel without
        # being too slow — Groq responses are fast, so this simulates
        # the natural pace of real token-by-token streaming
        if i % 3 == 2:
            await asyncio.sleep(0.02)
